...
serve(async (req) => {
  ...
  try {
    ...
    const { data: limitCheck, error: limitError } = await supabaseClient.rpc(
      "check_and_increment_chat", { p_user_id: user.id }
    );
    if (limitError) {
      console.warn("RPC error, continuing:", limitError);
...
${currentDateStr}`;
    ...
    if (!response.ok) {
      ...
      if (response.status === 402) {
        return new Response(JSON.stringify({ error: "AI kreditlari tugadi, iltimos to'ldiring." }), {
